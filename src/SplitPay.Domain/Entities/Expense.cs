...
public class SplitPart
{
    ...
    public Money Share { get; private set; } = null!;
    ...
    private void RecalculateShares(int participantsCountForEqual)
    {
        switch (Method)
        {
            case SplitMethod.Equal:
                ...
                var eachAmount = Math.Round(Amount.Amount / Math.Max(1, participantsCountForEqual), 2);
                var each = new Money(eachAmount, Amount.Currency);
            ...
            case SplitMethod.Percent:
                var percentSum = _parts.Sum(p => p.Percent ?? 0m);
                if (Math.Round(percentSum, 2) != 100m)
                    throw new InvalidOperationException("Percent shares must sum to 100%");
                foreach (var part in _parts)
                {
                    var shareAmount = Math.Round(Amount.Amount * ((part.Percent ?? 0m) / 100m), 2);
                    part.SetShare(new Money(shareAmount, Amount.Currency));
                }
                break;

            default:
                throw new InvalidOperationException("Unsupported split method");
        }
    }
}
